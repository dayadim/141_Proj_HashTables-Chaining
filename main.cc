...
void two_step_hashing() {
    cout << "==================== TIMING FOR DOUBLE HASHING ====================" << endl;

    vector<HashDataType> elements = gen_elements(size_ * 10, 141);
    Twox_hash<HashDataType, size_>* table = new Twox_hash<HashDataType, size_>;

    vector<double> average_insert_timings;
    vector<double> average_search_timings;

    int capacity = 0;
    int total_capacity = size_ * .4; // 40% capacity
    int time_increment = size_ / 10; // 10% increments
    ...
    while (capacity < size_ * 10) {
        ...
        cout << "(" << ((capacity) * 100.0 / size_) << "%): Average search time = "
    ...
    for (double timing : average_insert_timings) {
        cout << timing << "\n";
    }

    cout << "\nSearch (avg. ns/search):\n";
    for (double timing : average_search_timings) {
        cout << timing << "\n";
    }
}
